...
async def dmraid(xspam: Client, e: Message):
      """ Module: Dm Raid """
      PyroUserBot = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(PyroUserBot) == 2:
          ok = await xspam.get_users(PyroUserBot[1])
          id = ok.id
