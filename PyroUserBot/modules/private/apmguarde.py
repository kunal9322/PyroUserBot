...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await PyroUserBot.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await PyroUserBot.set_pm(True)
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await PyroUserBot.set_permit_message(PyroUserBot.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await PyroUserBot.set_permit_message(f"`{arg}`")
