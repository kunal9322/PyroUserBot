...
from PyroUserBot.database.pmpermitdb import get_approved_users, pm_guard
import PyroUserBot.database.pmpermitdb as PyroUserBot
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await PyroUserBot.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await PyroUserBot.set_block_message(PyroUserBot.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await PyroUserBot.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await PyroUserBot.get_pm_settings()
    await PyroUserBot.allow_user(chat_id)
